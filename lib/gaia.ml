...
let default_targets = {
  ocean_target = 0.20;      (* 20% ocean *)
  forest_target = 0.30;     (* 30% forest *)
  lava_target = 0.05;       (* 5% lava *)
  open_land_target = 0.45;  (* 45% open land *)
}

let create targets = {
  targets;
  smoothing_factor = 0.5;  (* Moderate adjustment speed *)
}
  ...
  let ocean_spread_chance =
    if ocean_error > 0.0 then
      ...
      calculate_adjustment 
        ...
        ~smoothing:gaia.smoothing_factor
    else if ocean_error < -0.05 then
      (* Way too much ocean: stop spreading *)
      0
    else
      (* Slightly too much or balanced *)
...
let status_report gaia board =
  ...
  let format_line (terrain, current) target =
    ...
    let status = 
      if abs_float diff < 2.0 then "✓"
      else if diff > 0.0 then "↑"
      else "↓"
    in
    Printf.sprintf "%s: %.1f%% (target: %.1f%%) %s" 
      name current_pct target_pct status
  in
  
  let lines = [
    "=== Gaia Status Report ===";
    format_line (List.find (fun (t, _) -> t = Ocean) distribution) gaia.targets.ocean_target;
    format_line (List.find (fun (t, _) -> t = Forest) distribution) gaia.targets.forest_target;
    format_line (List.find (fun (t, _) -> t = Lava) distribution) gaia.targets.lava_target;
    format_line (List.find (fun (t, _) -> t = Open_land) distribution) gaia.targets.open_land_target;
  ] in
