...
let init_history player_id behavior =
  { player_id; behavior; ages = []; unique_tiles = [] }
  ...
  List.iter
    (fun h ->
      ...
      Printf.fprintf oc "ages_%s = %s\n" label
        ...
        |> Printf.sprintf "[%s]");
      Printf.fprintf oc
        "plt.plot(times_%s, ages_%s, label='%s', linewidth=2)\n\n" label label
        label)
    data.histories;
...
let create_bar_plot data _filename =
  ...
  (* Prepare data *)
  let player_labels =
    List.map (fun h -> behavior_to_string h.behavior) data.histories
  in
  let tile_counts =
    List.map
      (fun h ->
        let final_tiles =
          if List.length h.unique_tiles > 0 then List.hd h.unique_tiles else 0
        in
        final_tiles)
      data.histories
  in
  ...
  Printf.fprintf oc "behaviors = %s\n"
    (String.concat ", " (List.map (Printf.sprintf "'%s'") player_labels)
    |> Printf.sprintf "[%s]");
  Printf.fprintf oc "tile_counts = %s\n"
    (String.concat ", " (List.map string_of_int tile_counts)
    |> Printf.sprintf "[%s]");
  Printf.fprintf oc "x_pos = np.arange(len(behaviors))\n\n";

  Printf.fprintf oc "# Create bar chart\n";
  Printf.fprintf oc
    "bars = plt.bar(x_pos, tile_counts, alpha=0.8, color=['#1f77b4', \
     '#ff7f0e', '#2ca02c'])\n";
  Printf.fprintf oc "plt.xticks(x_pos, behaviors)\n\n";

  Printf.fprintf oc "# Configure plot\n";
  Printf.fprintf oc
    "plt.title('Unique Tiles Visited by Each Player', fontsize=14, \
     fontweight='bold')\n";
