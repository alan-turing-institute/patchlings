...
let random_names = [
  "Ash"; "Sage"; "River"; "Storm"; "Blaze";
  "Echo"; "Frost"; "Luna"; "Raven"; "Sky"
]
...
let get_next_name () =
  let base_name = List.nth random_names (!name_counter mod List.length random_names) in
  let name = 
    if !name_counter >= List.length random_names then
      Printf.sprintf "%s%d" base_name ((!name_counter / List.length random_names) + 1)
    else
      base_name
  in
  incr name_counter;
