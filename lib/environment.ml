...
let get_player_env (board : Board.t) (coord_player_map : PlayerSet.t Board.CoordinateMap.t) (player : Player.t) =
  ...
  List.map
    (List.map
      (fun (step : int * int) ->
        ...
        let ps = Board.CoordinateMap.find adjacent_loc coord_player_map in
...
let serialise_env (env : t) =
  ...
  let flat_cells = Array.map (fun (x, y) -> env.(x).(y)) steps_in_order in
