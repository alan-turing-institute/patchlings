...
let get_player_env (board : Board.t) (coord_player_map : Player.Set.t Position.Map.t) (player : Player.t) =
  ...
  List.map
    (List.map
      (fun (step : int * int) ->
        ...
        let ps_opt = Position.Map.find_opt adjacent_loc coord_player_map in
        let ps = match ps_opt with
          | Some players -> Player.Set.remove player players
          | None -> Player.Set.empty in
